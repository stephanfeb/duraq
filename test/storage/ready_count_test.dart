...
void main() {
  Future<StorageUnderTest> openSqlite() async {
    final dir = Directory.systemTemp.createTempSync('duraq_ready_sqlite_');
    final storage = SQLiteStorage(dbPath: path.join(dir.path, 'duraq_test.db'));
    return (
      storage: storage,
      close: () async {
        storage.dispose();
  ...
  Future<StorageUnderTest> openIsar() async {
    final dir = Directory.systemTemp.createTempSync('duraq_ready_isar_');
    await ensureIsarCore();
    final isar = await Isar.open(
      IsarStorage.requiredSchemas,
      directory: dir.path,
      name: 'ready_count_test',
    );
  ...
  void readySuite(String backend, Future<StorageUnderTest> Function() open) {
