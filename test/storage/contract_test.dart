...
void main() {
  Future<StorageUnderTest> openSqlite({Duration? leaseDuration}) async {
    final dir = Directory.systemTemp.createTempSync('duraq_contract_sqlite_');
  ...
  void contractSuite(
    String backend,
    Future<StorageUnderTest> Function({Duration? leaseDuration}) open,
  ) {
  ...
  contractSuite('SQLite', openSqlite);
  contractSuite('Isar', openIsar);
}
