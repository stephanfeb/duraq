...
Everything else in this README applies unchanged: the backend implements the
same `StorageInterface` and is held to the same contract suite.
...
Run it from the repository root: it analyses and tests every package here, and
...
the test tree in `test/analysis_options.yaml`.

The storage contract is described once, in `test/support/`, and run against both
